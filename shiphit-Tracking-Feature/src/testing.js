...
import keys from "./keys"; // Assuming you have a keys file for URLs
...
const AwbTracking = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(keys.shiphit_url); // Replace with your actual script URL
        console.log('Data received from Google Sheet:', response.data);

        // Set initial form data from the first item in the response
  ...
  const handleAwbIdChange = (event) => {
    ...
    const matchedData = formData.find((entry) => entry.awbTrackingID === awbId);
